...
class AgentOrchestrator:
    ...
    def update_ram(self, session_id: str, ram_gb: int) -> Dict[str, Any]:
        ...
        return {
            "ram_gb": profile.total_gb, "used_gb": profile.used_gb,
            "available_gb": profile.available_gb, "usage_pct": profile.usage_pct,
            "max_concurrent_sub_agents": profile.max_concurrent_sub_agents,
            "sub_agents_per_main_agent": profile.sub_agents_per_main_agent,
            "recommendation": profile.recommendation, "agent_status": agent_status,
        }
    ...
    async def _run_sub_agent(self, agent_type: str, sub_agent: SubAgent, user_query: str, rag_context_str: str) -> str:
        # 🔽 FIXED: backslash \n ko f-string { } ke bahar nikal diya
        rag_context_display = 'Datasheet Context:\n\n' + rag_context_str if rag_context_str else 'No datasheet uploaded yet.'
        prompt = (
    ...
    async def _run_main_agent(self, agent_type: str, user_query: str, rag_context_str: str, active_indices: List[int], ram_gb: int = 8) -> AgentResponse:
        ...
        for idx in active_indices:
            if idx < len(all_subs):
                sub_data = all_subs[idx]
                name = sub_data["name"]
                active_subs.append(SubAgent(
                    agent_type=agent_type, index=idx, name=name,
                    description=sub_data["description"], active=True,
                ))

        if active_subs:
            tasks = [self._run_sub_agent(agent_type, sub, user_query, rag_context_str) for sub in active_subs]
        ...
        synthesis_messages = [
            ...
            {"role": "assistant", "content": f"Here are the reports from my sub-agents:\n\n{collation_text}\n\nNow I will synthesise these into a final response for the user."},
            {"role": "user", "content": user_query},
        ]
        ...
        return AgentResponse(
            agent_type=agent_type, content=final_response,
            sub_agent_reports=sub_agent_reports, ram_used=f"{ram_profile.total_gb}GB config",
            latency_ms=round(latency, 0),
        )

    # ── Public: Process a chat message ────────

    async def process_chat(self, session_id: str, user_query: str) -> AsyncGenerator[Dict[str, Any], None]:
        ...
        if len(q_stripped.split()) <= 2 or q_stripped in greeting_words:
            ...
            yield {"type": "status", "message": "Greeting detected — returning welcome message.", "ram": {"total_gb": profile.total_gb, "used_gb": profile.used_gb, "usage_pct": profile.usage_pct}}
            yield {"type": "agent_response", "agent": "nexus_cortex", "content": greeting_text, "sub_agent_reports": ["[System]: Greeting handled — no sub-agent dispatch needed."], "latency_ms": 0.0}
        ...
        yield {"type": "status", "message": f"Routing query to {len(main_agents)} main agents ({profile.sub_agents_per_main_agent} sub-agents each)...", "ram": {"total_gb": profile.total_gb, "used_gb": profile.used_gb, "usage_pct": profile.usage_pct}}

        tasks = [self._run_main_agent(agent_type=agent_type, user_query=user_query, rag_context_str=rag_context_str, active_indices=active_indices_map.get(agent_type, []), ram_gb=session.ram_gb) for agent_type in main_agents]
        ...
        for coro in asyncio.as_completed(tasks):
            try:
                ...
                yield {"type": "agent_response", "agent": response.agent_type, "content": response.content, "sub_agent_reports": response.sub_agent_reports, "latency_ms": response.latency_ms}
            except Exception as exc:
                logger.error("Main agent task failed: %s", exc, exc_info=True)

        session.chat_history.append({"role": "user", "content": user_query})
        for resp in responses:
            session.chat_history.append({"role": "assistant", "agent": resp.agent_type, "content": resp.content})
